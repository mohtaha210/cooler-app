...
# --- 1. دوال إدارة الملفات والتخزين الدائم ---
...
def generate_receipt_pdf(customer_name, date_str, items_data, grand_total, receipt_no):
    ...
    pdf.cell(sum(col_widths[1:]), 10, ar("المبلغ الإجمالي الكلي"), border=1, align='C', fill=True)
    pdf.ln(20)
...
st.set_page_config(page_title="نظام معمل برادات الرافدين", page_icon="❄️", layout="wide", initial_sidebar_state="collapsed")

# تحميل البيانات المحفوظة في حالة عدم وجودها بالذاكرة
    ...
    if not model_list:
        ...
        for model in model_list:
            ...
            with col_m1:
                st.write(f"**{model}**")
            with col_m2:
                qty = st.number_input("العدد المشتري:", min_value=0, value=0, key=f"rec_qty_{model}")
            with col_m3:
                price = st.number_input("سعر البراد الواحد:", min_value=0, value=0, step=5000, key=f"rec_price_{model}")
    ...
    if not model_list:
        ...
        if st.button("🚀 خصم المواد وتأكيد الإنتاج", type="primary", use_container_width=True):
            ...
            if missing_items:
                ...
                save_data() # حفظ التغيير دائمياً
                st.success(f"✅ تم تسجيل إنتاج ({count}) من [{model}] وخصم المواد وحفظ المخزون بنجاح!")
                st.rerun()
    ...
    with col_btn2:
        with st.popover("⚠️ تصفير جميع المواد في المخزن"):
            ...
            if st.button("نعم، أؤكد تصفير كافة الكميات", type="primary", use_container_width=True):
                ...
                save_data() # حفظ التصفير
                st.success("⚠️ تم تصفير كافة كميات المخزون وحفظ التعديل!")
                st.rerun()
    ...
    if st.button("➕ إضافة المادة للمخزن", type="primary", use_container_width=True):
        if new_item_name:
            if new_item_name in st.session_state.inventory:
                ...
                save_data() # حفظ المادة الجديدة
                st.success(f"✅ تمت إضافة المادة [{new_item_name}] وحفظها بنجاح!")
                st.rerun()
        else:
            st.error("يرجى إدخال اسم المادة.")

# --- Tab 6: أنواع البرادات (BOM) ---
with tabs[5]:
    ...
    if st.button("🛠️ حفظ النموذج الجديد", use_container_width=True):
        if new_model_name and selected_ingredients:
            ...
            save_data() # حفظ نموذج البراد الجديد
            st.success(f"✅ تم تعريف النموذج [{new_model_name}] وحفظه بنجاح!")
            st.rerun()
        else:
